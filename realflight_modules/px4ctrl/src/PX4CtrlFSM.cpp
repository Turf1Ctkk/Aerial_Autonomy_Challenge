...
void PX4CtrlFSM::process()
{
	...
	case AUTO_LAND:
	{
		...
		static double last_force_disarm_trial_time = 0.0; // Avoid too frequent calls
		...
		else
		{
			...
			if (print_once_flag)
			{
				ROS_INFO("\033[32m[px4ctrl] Land detected, force disarm after short idle.\033[32m");
	...
	controller.setPositionAdrcActive(state == CMD_CTRL, odom_data);
...
void PX4CtrlFSM::land_detector(const State_t state, const Desired_State_t &des, const Odom_Data_t &odom)
{
	...
	constexpr double POSITION_DEVIATION_C = -0.4; // Constraint 1: target position below real position for POSITION_DEVIATION_C meters.
	constexpr double VELOCITY_THR_C = 0.25;		  // Constraint 2: velocity below VELOCITY_MIN_C m/s.
	...
	else
	{
		if (tag_odom_data.recv_new_msg)
		{
			...
			else
			{
				...
				des.p(0) = param.takeoff_land.land_point_x;
				des.p(1) = param.takeoff_land.land_point_y;
				des.v.head(2) = Eigen::Vector2d::Zero();
				des.v += Eigen::Vector3d(0, 0, speed);
			}
		}
		else

...
namespace
{
double softDeadband(const double value, const double deadband)
{
  const double threshold = std::max(0.0, deadband);
  ...
  return (value >= 0.0 ? 1.0 : -1.0) * (magnitude - threshold);
}
}
...
Controller::Controller(Parameter_t &param_) : param(param_)
{
  ...
  PositionAdrc::Config pos_adrc_config;
  pos_adrc_config.axis_enable = param.pos_adrc.axis_enable;
  pos_adrc_config.beta1 = param.pos_adrc.beta1;
  pos_adrc_config.beta2 = param.pos_adrc.beta2;
  pos_adrc_config.beta3 = param.pos_adrc.beta3;
  pos_adrc_config.b0 = param.pos_adrc.b0;
  pos_adrc_config.fal_delta = param.pos_adrc.fal_delta;
  pos_adrc_config.leak_rate = param.pos_adrc.leak_rate;
  pos_adrc_config.max_dist_acc = param.pos_adrc.max_dist_acc;
  position_adrc_.configure(pos_adrc_config);
    ...
    double voltage)
{
  ...
  Eigen::Vector3d position_adrc_dist_acc = Eigen::Vector3d::Zero();
  Eigen::Vector3d position_adrc_comp_acc = Eigen::Vector3d::Zero();
  Eigen::Vector3d translational_acc = pid_error_accelerations + des.a;

  const ros::Time now = ros::Time::now();
  double position_adrc_dt = 1.0 / std::max(param.ctrl_freq_max, 1.0);
  if (last_position_adrc_timestamp_ != ros::Time(0))
  {
    position_adrc_dt = (now - last_position_adrc_timestamp_).toSec();
  }
  last_position_adrc_timestamp_ = now;

  if (param.pos_adrc.enable && position_adrc_runtime_active_)
  {
    if (!position_adrc_.initialized() || position_adrc_dt <= 0.0 || position_adrc_dt > 0.1)
    {
      resetPositionAdrc(odom);
      position_adrc_dt = 1.0 / std::max(param.ctrl_freq_max, 1.0);
    }

    position_adrc_dist_acc = position_adrc_.update(
        odom.p, odom.v, last_position_adrc_control_acc_, position_adrc_dt);

    Eigen::Vector3d compensated_dist_acc = Eigen::Vector3d::Zero();
    const double horizontal_deadband = std::max(0.0, param.pos_adrc.horizontal_deadband);
    if (horizontal_deadband > 0.0)
    {
      const Eigen::Vector2d horizontal_dist_acc = position_adrc_dist_acc.head<2>();
      const double horizontal_dist_norm = horizontal_dist_acc.norm();
      if (horizontal_dist_norm > horizontal_deadband)
      {
        compensated_dist_acc.head<2>() =
            horizontal_dist_acc * ((horizontal_dist_norm - horizontal_deadband) / horizontal_dist_norm);
      }
    }
    else
    {
      compensated_dist_acc(0) = softDeadband(position_adrc_dist_acc(0), param.pos_adrc.deadband[0]);
      compensated_dist_acc(1) = softDeadband(position_adrc_dist_acc(1), param.pos_adrc.deadband[1]);
    }

    compensated_dist_acc(2) = softDeadband(position_adrc_dist_acc(2), param.pos_adrc.deadband[2]);
    for (int i = 0; i < 3; ++i)
    {
      const double comp_gain = std::max(0.0, param.pos_adrc.comp_gain[i]);
      position_adrc_comp_acc(i) = -comp_gain * compensated_dist_acc(i);
    }

    translational_acc += position_adrc_comp_acc;
  }
  else
  {
    resetPositionAdrc(odom);
  ...
  last_position_adrc_control_acc_ = translational_acc;
  ...
  debug.adrc_dist_a_x = position_adrc_dist_acc(0);
  debug.adrc_dist_a_y = position_adrc_dist_acc(1);
  debug.adrc_dist_a_z = position_adrc_dist_acc(2);
  debug.adrc_comp_a_x = position_adrc_comp_acc(0);
  debug.adrc_comp_a_y = position_adrc_comp_acc(1);
  debug.adrc_comp_a_z = position_adrc_comp_acc(2);
...
void Controller::resetPositionAdrc(const Odom_Data_t &odom)
{
  position_adrc_.reset(odom.p, odom.v);
  last_position_adrc_timestamp_ = ros::Time(0);
  last_position_adrc_control_acc_.setZero();
}

void Controller::setPositionAdrcActive(const bool active, const Odom_Data_t &odom)
{
  if (position_adrc_runtime_active_ == active)
  {
    return;
  }

  position_adrc_runtime_active_ = active;
  resetPositionAdrc(odom);
}

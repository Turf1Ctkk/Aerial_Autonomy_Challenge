...
namespace diff_planner
{
  ...
  void PolyTrajOptimizer::addPVAJGradCost2CT(EIGENVEC &gdT, Eigen::VectorXd &costs, const int &K)
  {
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      for (int j = 0; j <= K; ++j)
      {
        ...
        // dynamic objects
        if (enable_dynamic_obs_ && objects_ != NULL && !objects_->states.empty())
        {
          gradp.setZero();
          costp = 0.0;
          gradt = 0.0;
          grad_prev_t = 0.0;
          const double sample_t = t + step * j;
          for (const auto& state : objects_->states)
          {
            ...
            if (obj_vel.norm() < dynamic_obs_min_speed_)
            {
              continue;
            }

            const double prediction_t = t_now_ - state.header.stamp.toSec() + dynamic_obs_time_offset_ + sample_t;
            if (prediction_t < 0.0 || prediction_t > dynamic_obs_prediction_time_)
            {
              continue;
            }
            obj_pos += prediction_t * obj_vel;

            const Eigen::Vector3d diff = pos - obj_pos;
            const Eigen::Vector3d rel_vel = vel - obj_vel;
            Eigen::Vector3d diff_for_dist = diff;
            Eigen::Vector3d rel_vel_for_dist = rel_vel;
            if (dynamic_obs_use_2d_)
            {
              diff_for_dist.z() = 0.0;
              rel_vel_for_dist.z() = 0.0;
            }

            const double cov_margin = dynamic_obs_uncertainty_scale_ *
                sqrt(std::max(0.0, state.poscov.x + state.poscov.y +
                                   (dynamic_obs_use_2d_ ? 0.0 : state.poscov.z)));
            const double clearance = dynamic_obs_clearance_ + cov_margin;
            const double dist_viola = clearance * clearance - diff_for_dist.squaredNorm();
            if (dist_viola > 0.0)
            {
              costp += dynamic_obs_weight_ * dist_viola * dist_viola * dist_viola;
              const Eigen::Vector3d grad_common =
                  dynamic_obs_weight_ * 3.0 * dist_viola * dist_viola * (-2.0) * diff_for_dist;
              gradp += grad_common;
              gradt += grad_common.dot(rel_vel_for_dist);
              grad_prev_t += grad_common.dot(-obj_vel);
            }
          }
  ...
  void PolyTrajOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    nh.param("optimization/enable_dynamic_obstacles", enable_dynamic_obs_, true);
    nh.param("optimization/dynamic_obstacle_weight", dynamic_obs_weight_, 8000.0);
    nh.param("optimization/dynamic_obstacle_clearance", dynamic_obs_clearance_, 1.4);
    nh.param("optimization/dynamic_obstacle_prediction_time", dynamic_obs_prediction_time_, 5.0);
    nh.param("optimization/dynamic_obstacle_time_offset", dynamic_obs_time_offset_, 0.12);
    nh.param("optimization/dynamic_obstacle_uncertainty_scale", dynamic_obs_uncertainty_scale_, 1.0);
    nh.param("optimization/dynamic_obstacle_min_speed", dynamic_obs_min_speed_, 0.0);
    nh.param("optimization/dynamic_obstacle_use_2d", dynamic_obs_use_2d_, true);
  }
  ...
  void PolyTrajOptimizer::setEnvironment(const GridMap::Ptr &map)
  {
    ...
    a_star_->initGridMap(grid_map_, Eigen::Vector3i(140, 140, 80));
  }

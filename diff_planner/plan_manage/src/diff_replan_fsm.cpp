...
namespace diff_planner
{
  ...
  void DiffReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    flag_hover_avoid_ = false;
    using_dynamic_temp_goal_ = false;
    ...
    nh.param("fsm/enable_dynamic_avoidance", enable_dynamic_avoidance_, true);
    nh.param("fsm/enable_dynamic_temp_goal", enable_dynamic_temp_goal_, true);
    nh.param("fsm/dynamic_object_topic", dynamic_object_topic_, std::string("/states"));
    nh.param("fsm/dynamic_collision_clearance", dynamic_collision_clearance_, 1.4);
    nh.param("fsm/dynamic_collision_prediction_time", dynamic_collision_prediction_time_, 5.0);
    nh.param("fsm/dynamic_collision_time_offset", dynamic_collision_time_offset_, 0.12);
    nh.param("fsm/dynamic_collision_uncertainty_scale", dynamic_collision_uncertainty_scale_, 1.0);
    nh.param("fsm/dynamic_collision_min_speed", dynamic_collision_min_speed_, 0.0);
    nh.param("fsm/dynamic_collision_use_2d", dynamic_collision_use_2d_, true);
    nh.param("fsm/dynamic_temp_goal_horizon", dynamic_temp_goal_horizon_, 0.6);
    nh.param("fsm/dynamic_temp_goal_trigger_distance", dynamic_temp_goal_trigger_distance_, 1.4);
    nh.param("fsm/dynamic_temp_goal_min_force", dynamic_temp_goal_min_force_, 1e-3);
    nh.param("fsm/dynamic_temp_goal_repulsion_gain", dynamic_temp_goal_repulsion_gain_, 10.0);
    nh.param("fsm/dynamic_temp_goal_return_distance", dynamic_temp_goal_return_distance_, 0.4);
    ...
    obj_sub_ = nh.subscribe<obj_state_msgs::ObjectsStates>(dynamic_object_topic_, 100, &DiffReplanFSM::objectCallback, this);
  ...
  void DiffReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case WAIT_TARGET:
    {
      if (!have_target_ || !have_trigger_)
      {
        ...
        if (enable_dynamic_avoidance_ && enable_dynamic_temp_goal_)
        {
          planDynamicTempGoal(true);
        }
        goto force_return; // return;
      }
      else
    ...
    case SEQUENTIAL_START: // for swarm or single drone with drone_id = 0
    {
      if (planner_manager_->pp_.drone_id <= 0 || (planner_manager_->pp_.drone_id >= 1 && have_recv_pre_agent_))
      {
        ...
        else
        {
          ...
          {
            if (planDynamicTempGoal(false))
            {
              replan_fail_count_ = 0;
              changeFSMExecState(EXEC_TRAJ, "DYN_TEMP");
            }
            else
            {
    ...
    case EXEC_TRAJ:
    {
      ...
      else if ((using_dynamic_temp_goal_ || flag_hover_avoid_) &&
               (final_goal_ - pos).norm() < dynamic_temp_goal_return_distance_)
      {
        const Eigen::Vector3d resume_goal = saved_final_goal_;
        using_dynamic_temp_goal_ = false;
        flag_hover_avoid_ = false;
        planNextWaypoint(resume_goal, true);
      }
      else if ((t_cur > info->duration - 1e-2) && touch_the_goal) // case 3: the final waypoint reached
      {
        if (using_dynamic_temp_goal_ || flag_hover_avoid_)
        {
          const Eigen::Vector3d resume_goal = saved_final_goal_;
          using_dynamic_temp_goal_ = false;
          flag_hover_avoid_ = false;
      ...
      for (size_t j = j_start; j < pts_chk[i].size(); ++j)
      {
        ...
        for (size_t id = 0; id < planner_manager_->traj_.swarm_traj.size(); id++)
        {
          ...
          if (t_X > 0 && t_X < planner_manager_->traj_.swarm_traj.at(id).duration)
          {
	        ...
	        {
	          for (const auto& state : planner_manager_->traj_.objs.states)
	          {
	            ...
	            if (obj_vel.norm() < dynamic_collision_min_speed_)
	            {
	              continue;
	            }
	            const double prediction_t = info->start_time - state.header.stamp.toSec() +
	                                        dynamic_collision_time_offset_ + t;
	            if (prediction_t < 0.0 || prediction_t > dynamic_collision_prediction_time_)
	            {
	              continue;
	            }
	            obj_pos += prediction_t * obj_vel;
	            Eigen::Vector3d diff = p - obj_pos;
	            if (dynamic_collision_use_2d_)
	            {
	              diff.z() = 0.0;
	            }
	            const double cov_margin = dynamic_collision_uncertainty_scale_ *
	                sqrt(std::max(0.0, state.poscov.x + state.poscov.y +
	                                     (dynamic_collision_use_2d_ ? 0.0 : state.poscov.z)));
            const double allowed_dist = dynamic_collision_clearance_ + cov_margin;
	            if (diff.norm() < allowed_dist)
	            {
	              ROS_WARN("drone %d detect collision with dynamic object, dist=%f, clearance=%f",
	                       planner_manager_->pp_.drone_id, diff.norm(), allowed_dist);
	              dangerous = true;
	              dangerous_dynamic = true;
	              break;
	            }
	          }
	        }
          ...
          if (planFromLocalTraj()) // Make a chance
          {
            ...
            return;
	          ...
	          {
	            if (dangerous_dynamic && planDynamicTempGoal(false))
	            {
	              ROS_INFO("Plan dynamic temporary goal when detect object collision. %f", t / info->duration);
	              changeFSMExecState(EXEC_TRAJ, "DYN_SAFETY");
	              return;
	            }
  ...
  bool DiffReplanFSM::computeDynamicTempGoal(Eigen::Vector3d &temp_goal)
  {
    if (!enable_dynamic_avoidance_ || !enable_dynamic_temp_goal_ ||
        planner_manager_->traj_.objs.states.empty() || !have_odom_)
    {
      return false;
    }
    ...
    for (const auto& state : planner_manager_->traj_.objs.states)
    {
      ...
      const double state_age = std::max(0.0, now - state.header.stamp.toSec() + dynamic_collision_time_offset_);
      if (state_age > dynamic_collision_prediction_time_)
      {
        continue;
      }
      obj_pos += state_age * obj_vel;

      Eigen::Vector3d rel = odom_pos_ - obj_pos;
      if (dynamic_collision_use_2d_)
      {
        rel.z() = 0.0;
      }
      ...
      const double lookahead = std::max(0.0, std::min(dynamic_collision_prediction_time_ - state_age,
                                rel.dot(obj_vel) / std::max(obj_vel.squaredNorm(), 1e-6)));
      Eigen::Vector3d future_rel = rel - lookahead * obj_vel;
      if (dynamic_collision_use_2d_)
      {
        future_rel.z() = 0.0;
      }
      ...
      const double cov_margin = dynamic_collision_uncertainty_scale_ *
          sqrt(std::max(0.0, state.poscov.x + state.poscov.y +
                             (dynamic_collision_use_2d_ ? 0.0 : state.poscov.z)));
      const double trigger_dist = dynamic_temp_goal_trigger_distance_ + cov_margin;
      if (future_rel.norm() > trigger_dist && dist > trigger_dist)
      {
        continue;
      }

      const double approach_speed = std::max(0.0, obj_vel.dot(rel.normalized()));
      force += dynamic_temp_goal_repulsion_gain_ * (approach_speed + 1e-3) /
               (dist * dist + 1e-3) * rel;
    }

    if (force.norm() < dynamic_temp_goal_min_force_)
    {
      return false;
    }

    Eigen::Vector3d dir = force.normalized();
    if (dynamic_collision_use_2d_)
    {
      dir.z() = 0.0;
      if (dir.norm() < 1e-3)
    ...
    for (double scale = 1.0; scale >= 0.2; scale -= 0.2)
    {
      Eigen::Vector3d candidate = odom_pos_ + scale * dynamic_temp_goal_horizon_ * dir;
      if (dynamic_collision_use_2d_)
      {
        candidate.z() = odom_pos_.z();
      }
      if (!planner_manager_->grid_map_->getInflateOccupancy(candidate))
      {
        temp_goal = candidate;
        return true;
      }
  ...
  bool DiffReplanFSM::planDynamicTempGoal(bool from_wait_state)
  {
    Eigen::Vector3d temp_goal;
    if (!computeDynamicTempGoal(temp_goal))
    {
      return false;
    }

    const bool already_temp = using_dynamic_temp_goal_;
    if (!already_temp)
    {
      saved_final_goal_ = from_wait_state || !have_target_ ? hover_pos_ : final_goal_;
      using_dynamic_temp_goal_ = true;
      flag_hover_avoid_ = from_wait_state;
    }

    if (!planNextWaypoint(temp_goal, false))
    {
      if (!already_temp)
      {
        using_dynamic_temp_goal_ = false;
    ...
    if (from_wait_state)
    {
      changeFSMExecState(GEN_NEW_TRAJ, "DYN_WAIT");
      return true;
    }
  ...
  void DiffReplanFSM::objectCallback(const obj_state_msgs::ObjectsStatesConstPtr &msg)
  {
